...
def load_data_from_csv(csv_data):
    ...
    # csv_data = csv_data[['assembly_content', 'label']]
    data_list = []
...
def train(model, train_data_loader, val_data , tokenizer, device, args):
    ymd, hms = get_date()  
    # dataset_name = args.train_filename.split('/')[1]
    ...
    for epoch in range(args.num_epochs):
        ...
        if eval_params['accuracy'] > best_accuracy:
            torch.save(model.state_dict(), os.path.join(str(checkpoint_path), 'best-acc-model.bin'))    
...
def evaluate(model, device, tokenizer, val_data , args):
    ymd, hms = get_date()  
    # dataset_name = args.train_filename.split('/')[1]
    args_path = os.path.join(args.output_dir, 'args', ymd + '-' + hms)
    
    tensorboard_path = os.path.join(args.output_dir, 'tbd-logs', 'ContraMLM', ymd, hms)
    # logger.info("Best accuracy checkpoint path created in: %s", checkpoint_path)
    path_dict = {'args': args_path}
    makedir(path_dict)
    writer = SummaryWriter(log_dir=str(tensorboard_path))
    args_dict = vars(args)
    dataset_path = args.dataset
    dataset_path = dataset_path.split('/')[-2]
    with open(os.path.join(str(args_path), 'args.json'), 'w') as f:
        json.dump(args_dict, f)
    ...
    for epoch in range(args.num_epochs):
        ...
        eval_steps = 0
        with torch.no_grad():
            pbar = tqdm(eval_data_loader, mininterval=5)
            for idx , batch in enumerate(pbar):
                input_data , target  = batch
                input_encoding, label = convert_to_encoding(input_data,target, tokenizer,
                                                            args.max_token_length)  # 处理每一批次，转换为batchEncoding
                input_encoding_gpu = input_encoding.to(device)
                label_gpu = label.to(device)
        ...
        eval_params = {'accuracy': accuracy, 'precision': precision_pos, 'recall': recall_pos, 'f1_score': f1_score_pos,
                'avg_loss': avg_loss}
        if eval_params['accuracy'] > best_accuracy:
            best_accuracy = eval_params['accuracy']
        if eval_params['precision'] > best_precision:
        ...
        # writer.add_scalar('/Evaluation/Avg Loss', eval_params['avg_loss'], epoch)
    writer.add_hparams(
        {'max_token_length': args.max_token_length,  'lr': '', 'bsize': args.batch_size,
         'epochs': args.num_epochs, 'pretrained_model': 'ContraMLM_O2_Od_Ox', 'seed': args.seed,'dataset':dataset_path,},
        {
            'best_accuracy': best_accuracy,
            'best_precision': best_precision,
            'best_recall': best_recall,
...
def main():
    ...
    # parser.add_argument("--valid_filename", default='', type=str, required=False,
    #                     help="test data file name")
    parser.add_argument("--pretraining_model_path", type=str, required=True,
    ...
    parser.add_argument("--dataset", default='', type=str, required=False,
    ...
    args.output_dir = 'output/valid/cross_valid'
    ...
    model.load_state_dict(torch.load(args.pretraining_model_path, weights_only=True),strict=False)
    evaluate(model, device, tokenizer , data , args )
    
    
    # 验证所有 CWE 类型
    CWE_root_dir = '/data/jlzhao/model_Roberta/csv_data/CWE_all'
    for cwe_data in os.listdir(CWE_root_dir):
        data_path = os.path.join(CWE_root_dir , cwe_data)
        args.dataset = data_path
        data = pd.read_csv(data_path)
        evaluate(model, device, tokenizer , data , args )
